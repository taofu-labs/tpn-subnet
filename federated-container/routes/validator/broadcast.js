...
router.post( '/workers', async ( req, res ) => {
    ...
    const handle_route = async () => {
        ...
        const { clashing_workers, non_clashing_workers } = await find_clashing_workers( { workers } )

        // For the clashing workers, find the first matching winner
        const cleared_clashing_workers = await find_first_valid_workers_by_ip( { workers: clashing_workers } )
